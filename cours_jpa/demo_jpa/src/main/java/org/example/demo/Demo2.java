...
public class Demo2 {
    ...
    public static void main() {
        ...
        Person person2 = em.getReference(Person.class,4L);
        System.out.println(person2);

        // Récupération avec Query

        // createQuery single result
        Query query = em.createQuery("select p from Person p where p.nom = 'Dupont'");
        Person person3 = (Person) query.getSingleResult();
        System.out.println(person3);

        // createQuery result list
        System.out.println("liste de personne avec id supérieur à 4");
        Query query1 =em.createQuery("select p from Person p where p.id > 5");
        ...
        // Suppression
        System.out.println("Suppression d'une occurence (personne avec l'id 12");
        transac.begin();
        Person person5 = em.find(Person.class,12L);
        em.remove(person5);
        transac.commit();
